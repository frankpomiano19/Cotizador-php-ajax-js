...
function get_items()
{
    ...
    //Si no existe la cotizacion y obviamente está vació el array
...
function get_module($view, $data = [])
{
    ...
    $d = $data = json_decode(json_encode($data)); //conversion a objeto
...
function hook_restart_quote()
{
    ...
    if (!restart_quote()) {
        json_output(json_build(400, null, 'Hubo un problema al reiniciar la cotizacion.'));
    }

    json_output(json_build(200, get_quote(), 'La cotizacion se ha reiniciado con éxito.'));
}
...
function hook_delete_concept()
{
    ...
    if (!delete_item((int) $_POST['id'])) {
        json_output(json_build(400, null, 'Hubo un problema al borrar un concepto.'));
    }
...
function generate_pdf($filename = null, $html, $save_to_file = true)
{
    ...
    $pdf = new Dompdf();
    //soluciona el problema de no incluir el logo de la empresa
    $pdf->getOptions()->setChroot(getcwd() . '/assets');
...
function hook_send_quote()
{
    ...
    $body = '<h1>Nueva cotización</h1><br><p>Hola <b>%s</b>, haz recibido una cotización con folio <b>%s</b> por parte  de <b>%s</b>, se encuentra
    adjunta a este correo.</p>';
